...
| **连接与隐私设置** | ⚠️ 桌面精简（代理/托盘） | ⚠️ **缺** KS / 保护等级 / 开机自连 / 省电白名单 UI |
| **Android 独有能力入口** | ❌ 产品不对齐到桌面 | 🚧 **待补**（分应用、KS 等） |
...
| **Kill Switch** / 断网保护 UI | ❌ UI 隐藏 | 📋 应对齐原生设置页 |
| **分应用直连** | ❌ | 📋 应对齐入口与列表页 |
| **开机自连** | ❌ | 📋 |
| **保护等级 / 省电白名单 / Always-on VPN** | ❌ | 📋 |
| **FCM** | ❌ | 可选（P2） |
...
| **分应用直连** | ✅ | ❌ **缺完整页**（Android 入口占位「即将开放」） | ❌ |
| 连接与隐私 | ✅ 完整 | ⚠️ 桌面精简；Android 文案/开关已分支，KS/开机自连待插件 | ⚠️ |
| 诊断日志 | ✅ | ✅（1.2 起非仅桌面） | ✅ |
| 关于 | ✅ | ✅ | ✅ |
...
| 规则直连 | ✅ | ✅ | ✅（前端+配置） | ✅ |
| 分应用直连 | ✅ | ❌ | 📋 UI/插件 | ❌ |
| Kill Switch | ✅ | 代码有 / UI 关 | 📋 UI | 说明 |
| 自动重连 | ✅ 完整重连 | ✅ 完整重连 | 应对齐插件策略 | 部分 |
| 开机自连 | ✅ | ❌ | 📋 | ❌ |
...
|----|--------|----------------|------|
| 分应用直连页 | ✅ | ❌ 无路由 | 补路由 + 调 Android 插件列包名 |
| 连接与隐私：KS / 开机自连 / 保护等级 / 省电 | ✅ | 仅自动重连+隐私自检（桌面托盘） | 按 `detectClientPlatform()==='android'` 展示 Android 设置块 |
