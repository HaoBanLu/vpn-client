...
最后核对：**2026-09-22**（版本线 `1.2.39` / code `159`）

| 功能 / 变更 | 状态 | 日期 | 备注 |
|-------------|------|------|------|
| 发版 1.2.39 / code 159 | ✅ | 2026-09-22 | OTA 改回 DownloadManager + 轮询收尾/量级误判修复；连接大按钮四字文案与图标统一；稍后再说仍可在关于页下载。Tag `v1.2.39` |
| Android OTA 改回 DownloadManager | ✅ | 2026-09-22 | 恢复系统 DownloadManager；补轮询收尾；修 versionCode 量级误判（159 vs 1002038）导致 APK 被当过期删除。模拟器：提示/稍后再说/下载→pending 通过 |
| 连接页大按钮图标/文案统一 | ✅ | 2026-09-22 | 一键连接/断开连接/正在连接/正在重连 四字对齐；统一 44px SVG 图标（闪电/护盾/转圈） |
| 发版 1.2.38 / code 158 | ✅ | 2026-09-22 | 升级后加载卡死修复；节点延迟「超时」可点重试；按钮 loading 圈动画；流量日期展示。Tag `v1.2.38` |
| 登录等按钮 loading 圈不转 | ✅ | 2026-09-22 | `ky-btn__spinner` 引用缺失的 `@keyframes ky-spin`；补 keyframes，并给 `anticon-spin`（桌面刷新图标）同一套旋转。KySpin / 下拉刷新 / 节点连接钮本身 keyframes 正常 |
| 节点响应「—」体验差 | ✅ | 2026-09-22 | 成功测速不再因 &lt;8ms 被藏成「—」；失败显示「超时」可点重试；本机探测失败自动重试一次。「最快」仍对噪声延迟降权 |
| 流量统计日期去掉 T/Z | ✅ | 2026-09-22 | `TrafficView` 用 `formatDateOnly`；依赖控制面日流量返回 `YYYY-MM-DD` |
| 覆盖安装后四 Tab 一直加载 | ✅ | 2026-09-22 | 残留 VPN 被当成 connected → API 失败不拆隧道；WebView 请求偶发永不回调。升级 90s 宽限期强制拆隧道、disconnect/账户/节点/套餐硬超时、MainShell 拆隧道最多等 5s。随 1.2.38 发版 |
| 发版 1.2.37 / code 157 | ✅ | 2026-09-21 | Android OTA 物理网下载；节点扁平列表+按钮连接；响应延迟去噪；套餐卡对齐会员中心并当前套餐置顶。Tag `v1.2.37` |
| 套餐页对齐会员中心卡面 | ✅ | 2026-09-21 | `KyPackageCard` 大价+权益清单+通栏 CTA；去掉顶部摘要条；当前套餐卡片置顶并标「当前套餐」 |
| Android OTA 下载改应用内 HTTP + 物理网 | ✅ | 2026-09-21 | VPN 下系统 DownloadManager 常失败；改为 `HttpURLConnection`，优先 `Network.openConnection` 绑物理网；相对 `download_url` 用 `VITE_API_BASE_URL` 拼绝对地址；进度事件 `download-progress` |
| 节点列表去圆角通栏扁平化 | ✅ | 2026-09-21 | 对齐 KyListItem 扁平行；去掉选中圆角块；仅点「连接/切换」按钮才连接 |
| 连接页去掉无参考价值的「入口 xxms」 | ✅ | 2026-09-21 | Hero 改为「响应 xxms」（不用「延迟」）；过滤 <8ms 的机房/模拟器噪声，节点列表不再刷假 1ms |
| 发版 1.2.36 / code 156 | ✅ | 2026-09-20 | 节点页边距统一、地区 Chip 筛选（含全部）、下拉刷新软失败。Tag `v1.2.36` |
...
| 连接期吓人 toast / 先失败后成功 | ✅ | 2026-09-20 | 软探针误报不拆隧；建隧抑制网络 toast；已连接时刷新失败不再写入 store.error；已保护/连接中隐藏底部红字错误区 |
| 状态通知展开为分行指标 | ✅ | 2026-09-20 | 与连接页共用 VpnSessionStatsTracker；收起一行摘要，展开显示下载/上传/时长/累计 |
| 升级检查改前台即时复检 | ✅ | 2026-09-20 | 启动必查；每次显示 App（visibility visible）复检，冷却从 30 分钟改为 60 秒 |
| 节点页左栏地区 + 自动测速 | ✅ | 2026-09-20 | 初版左栏 Chip；后改为通讯录式 |
| 节点页边距与刷新软失败 | ✅ | 2026-09-20 | 恢复与其它 Tab 一致的左右留白；地区粘性头加强层次；下拉刷新不再先清缓存，已有列表时网络超时软失败不盖整页。地区 Chip 含「全部」并真正筛选列表。模拟器验证 |
| 节点页通讯录式列表 | ✅ | 2026-09-20 | 按地区粘性分区标题 + 右侧索引跳转；进页自动测速；整行可点连接（边距见上条） |
...
| 节点批量测速本机优先 | ✅ | 2026-08-17 | 不再先等控制面整批；本机 TCP 并发 8、边测边出；合并用本机 RTT，避免机房 1ms 盖过真实延迟；控制面仅补本机失败 |
| 发版 1.2.25 / code 145 | ✅ | 2026-08-13 | 覆盖安装登录前拆残留 VPN；登录超时拆后重试；回前台立刻心跳（前台 15s）。Tag `v1.2.25` |
| 覆盖安装登录超时与踢线延迟 | ✅ | 2026-08-13 | 冷启动/登录前拆残留系统 VPN；版本变化或系统仍有 VPN 即拆；登录网络失败拆后重试；回前台立刻心跳、前台 15s |
| 发版 1.2.24 / code 144 | ✅ | 2026-08-13 | 「我的」充值+订单+客服 Telegram 主入口；覆盖安装拆残留 VPN 避免「网络异常」。Tag `v1.2.24` |
| 「我的」菜单：充值+订单+客服直达 | ✅ | 2026-08-13 | 账户只留充值/订单；订单页 Tab 充值|套餐；旧充值/购买记录路由重定向；客服页 Telegram 主按钮，工单收入客服页；iOS 同步 |
| 覆盖安装后网络异常误判 | ✅ | 2026-08-13 | 升级时拆除残留系统 VPN 再拉账户；网络失败再断隧道重试；失败页可「重新登录」。401 仍自动回登录，不断网超时不自动登出 |
| 连接页与通知栏速率 SSOT | ✅ | 2026-08-13 | Android `VpnSessionStatsTracker` 统一 warmup/EMA/cap；通知独占 sampleRates，`getStats` 只 peek；格式化与连接页同阈值；未连接不拉 `vpn_stats`；删空的 `vpn://stats` listen。累计同源、速率约 1s 刷新+EMA，非内核推送；与套餐 GB 不对账 |
| 「我的」子页统一顶栏与宽度 | ✅ | 2026-08-13 | KySubPage 钉住 48px 顶栏、单层 16px 边距；修应用直连授权刷新、客服外开 Telegram、连接与隐私保护状态。模拟器已验；未装 TG 时走系统浏览器，禁止 tg:// 落入 WebView |
| 加载超时与会话三态 | ✅ | 2026-08-13 | 超时/断网显示页内重试，不再误显示「暂无套餐」；refresh 去重；logout 清账户缓存；401 才回登录；iOS Connect/Profile 同步空态 |
| 发版 1.2.22 / code 142 | ✅ | 2026-08-13 | 打开即显示状态通知（连接后流量）；禁用网页选中/缩放；修复连接闪失败；「我的」文案清理 |
| 发版 1.2.21 / code 141 | ✅ | 2026-08-12 | 真机状态栏原生 insets；Toast 降噪/去重；连接前台通知对齐 Compose（节点/速率/时长/断开） |
| 发版 1.2.20 / code 140 | ✅ | 2026-08-12 | Android 已连无网（禁自身 disallow + TUN blocking/gvisor）；状态栏深色图标；节点页顶栏固定 |
